import AWS, { CostExplorer } from "aws-sdk"; //npm i aws-sdk
...
export const create = async (req, res) => {
  ...
  try {
    ...
    if (alreadyExist) return res.status(400).send("Title is taken.Try another");
...
export const publishCourse = async (req, res) => {
  try {
    ...
    console.log(err);
    res.status(400).send("publish failed");
  }
};
export const unpublishCourse = async (req, res) => {
  try {
    ...
    console.log(err);
    res.status(400).send("unpublish failed");
  }
};
...
export const checkEnrollment = async (req, res) => {
  ...
  let ids = [];
  //because some users may not be enrolled in any course
  let length = user.courses && user.courses.length;
  for (let i = 0; i < length; i++) {
    ids.push(user.courses[i].toString());
  }
  //checking if courseId is in the ids array
  res.json({
...
export const freeEnrollment = async (req, res) => {
  try {
    ...
    console.log("free enrollment failed", err);
